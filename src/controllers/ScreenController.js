...
export default function screenController() {
  ...
  const submitForm = (event) => {
    ...
    switch (modalId) {
        ...
        todoListController(userName).then((data) => {
          ...
          setTimeout(() => {
            ...
            if (defaultProject === "on" && user.projects.length === 0) {
              tdController.createProject(
                "default",
                `Default project for user ${userName}`,
              );
            }
            initialize();
          }, 100);
      ...
      case "createPjModal": {
        ...
        currentProject = tdController.createProject(
          projectTitle,
          projectDescription,
        ).currentProject;
      ...
      case "editPjModal":
        ...
        document.querySelector(".project-info").querySelector("p").textContent =
      ...
      case "createTaskModal": {
        ...
        const objTask = tdController.createTask(
          taskTitle,
          taskDescription,
          taskDueDate,
          taskPriority,
  ...
  const setButtonTooltip = (fieldId) => {
    const btnConf = tooltipConfigurations.find(
      (tooltip) => tooltip.id === fieldId,
    );
    ...
    if (user) {
      ...
      news
        ...
        .addEventListener("click", () =>
          showNotificationModal(getDueSoonTasks(user)),
        );
      ...
      user.projects.forEach((project) => {
        project.tasks.filter((task) => {
          ...
          const timeDifference =
            (taskDueDate.getTime() - today.getTime()) / (1000 * 3600 * 24);
  ...
  const setContentArea = () => {
    ...
    if (currentProject) {
      ...
      editProjectBtn.querySelector("button").addEventListener("click", () => {
      ...
      deleteProjectBtn.querySelector("button").addEventListener("click", () => {
  ...
  const setTasks = () => {
    ...
    taskCreateBtn
      .querySelector("button")
      .addEventListener("click", () => createModal("createTaskModal"));
  ...
  const showHeaderActionModal = (e) => {
    const modalOption =
      e.target.parentElement.querySelector(".tooltip-text").innerHTML;
  ...
  const setTaskLocation = (task) => {
    ...
    deleteBtn.querySelector("button").addEventListener("click", (event) => {
      currentTask = tdController.selectTask(
        event.target.parentElement.parentElement.parentElement.dataset.id,
      ).currentTask;
      createModal("deleteTaskModal");
    });
  ...
  const handleDragStart = (event) => {
    currentTask = tdController.selectTask(
      event.target.querySelector("h3").textContent,
    ).currentTask;
  ...
  const handleDrop = (event) => {
    ...
    const color = taskElement
      .querySelector(".traffic-light")
      .querySelector("div");
